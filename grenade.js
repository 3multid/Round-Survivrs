class Grenade {
    ...
    throw(){
        this.timeCount = 0.5 + dist(this.targetX, this.targetY, this.x, this.y) / 250;
    ...
    fly(){
        if(dist(this.x, this.y, this.targetX, this.targetY) <= this.dir.mag() / 2 || this.exploded || hitBorder(this)){
        ...
        if(!this.exploded){
            ...
            for(let i = 1; i <= 2; i++){
                if(hit(player[i], this)){
                    player[i].hp -= 45 - dist(player[i].x, player[i].y, this.x, this.y) / this.radius * 30; 
                }
            }
            for(let i = 0; i < 6; i++){
                let rd = random(PI / 3);
                let ang = PI / 3 * i + rd;
                let bl = new Bullet(0, createVector(cos(ang), sin(ang)), this.x, this.y);
                bl.shoot();
                bullet.push(bl);
            }
        }
